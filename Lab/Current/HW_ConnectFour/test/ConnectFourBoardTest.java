...
public class ConnectFourBoardTest {
  ...
  public void testIsWinnerVertical() {
    System.out.println("Testing isWinnerVertical()...");
    ConnectFourBoard b = new ConnectFourBoard();
    //int[] moves = {1, 2, 1, 2, 1, 2, 1}; //fails
    //int[] moves = {1, 2, 2, 1, 2, 1, 2, 1, 2, 1}; /passes
    //bound issue with bottom row
    int[] moves = {};
    for (int i = 0; i < moves.length; i++) {
      b.makeMove(moves[ i]);
    }
  ...
  public void testIsWinnerDiagonal() {
    System.out.println("Testing isWinner()...");
    ConnectFourBoard b = new ConnectFourBoard();
    int[] moves = {1, 2, 2, 3, 4, 3, 3, 4, 5, 4, 4};
    for (int i = 0; i < moves.length; i++) {
      b.makeMove(moves[ i]);
    }
    assertTrue(b.isWinner());
