...
/**
 Create a window in which simple graphic elments can be drawn. Also
 allow the programmer to easily get user mouse and keyboard inputs.

 @author Dr. Wayne Brown <Wayne.Brown@usafa.edu>

 @version 1.0 July 2007 Original modification of textbook class

 @version 1.1 19 Sept 2007 Added additional user input for mouse and
 keyboard
 ...
 ============================================================================
 Original Documentation by Stuart Reges and Marty Stepp (textbook
 authors) 07/01/2005

 The DrawingPanel class provides a simple interface for drawing
 persistent images using a Graphics object. An internal BufferedImage
 object is used to keep track of what has been drawn. A client of the
 class simply constructs a DrawingPanel of a particular size and then
 draws on it with the Graphics object, setting the background color if
 they so choose.
 ...
 1) Draws to an "offscreen graphics" window that use user must then copy
 to the screen. 2) The GUI runs in a separate thread to allow for the
 suspension and resumption of the application program (so it can wait
 for mouse and key events). 3) Removed the timer that refreshed the
 graphics image.
 ============================================================================
 */
import java.awt.*;
...
public class DrawingPanel extends Thread implements MouseInputListener, KeyListener {
  ...
  //----------------------------------------------------------------------------  
  /**
   Construct a drawing panel of a given width and height enclosed in a
   window.
   */
  ...
  //----------------------------------------------------------------------------  
    /*
   Run the GUI drawing panel in a separate thread so that the
   application that is drawing to the window can be paused and restarted
  ...
  //----------------------------------------------------------------------------  
  /**
   Internal method to search for a key to see if it is in the "active
   key" list (applications will never call this method).
   */
  ...
  /**
   ...
   Known problem: if multiple keys are being held down simultaneously,
   the operating system does not always generate new key pressed events
   for new keys pressed.
   */
  ...
  //----------------------------------------------------------------------------  
  /**
   Obtain the Graphics object needed to draw on the DrawingPanel's
   offscreen graphics buffer. Make sure you call copyGraphicsToScreen()
   after all your drawing methods have been called to copy the offscreen
  ...
  //----------------------------------------------------------------------------  
  /**
   Copy the offscreen graphics buffer to the screen. No graphics are
  ...
  //----------------------------------------------------------------------------  
  /**
   Make your application "sleep" for the specified number of
   milliseconds.
   */
  ...
  //----------------------------------------------------------------------------  
  /**
   Wait for a mouse click on a specific mouse button (your application
   goes to sleep until the specific mouse button has been clicked
  ...
  //----------------------------------------------------------------------------  
  /**
   Determine if a mouse click has occurred. Calling this method always
   resets the status of the specified button to "unclicked." If you want
   the location of the click, immediately call
   ...
   @return true if a mouse click using the specified button has occurred
   since the last call to this method, false otherwise.
   */
  ...
  //----------------------------------------------------------------------------  
  /**
   Determine if a specific mouse button is down at the time of the
   method call.
  ...
  /**
   ...
   @return button code (DrawingPanel.LEFT_BUTTON,
   DrawingPanel.MIDDLE_BUTTON, or DrawingPanel.RIGHT_BUTTON)
   */
  ...
  /**
   ...
   @return x coordinate of the mouse in pixels (left side of window is
   0, right side is the (DrawingPanel's width - 1).
   */
  ...
  /**
   ...
   @return y coordinate of the mouse in pixels (top of window is 0,
   bottom is the (DrawingPanel's height - 1).
   */
  ...
  //----------------------------------------------------------------------------  
  /**
   Get the x coordinate of the mouse's location when the most recent
   mouse click of the specified button occurred. Call the
   mouseClickHasOccurred(whichButton) method to determine if a mouse
   click has occurred. If no mouse click has occurred, this method
   returns a bogus x coordinate.

   @return x coordinate of the mouse (in pixels) when the most recent
   mouse click for the specified button occurred.
   */
  ...
  //----------------------------------------------------------------------------  
  /**
   Get the y coordinate of the mouse's location when the most recent
   mouse click of the specified button occurred. Call the
   mouseClickHasOccurred(whichButton) method to determine if a mouse
   click has occurred. If no mouse click has occurred, this method
   returns a bogus y coordinate.

   @return y coordinate of the mouse (in pixels) when the most recent
   mouse click for the specified button occurred.
   */
  ...
  //----------------------------------------------------------------------------  
  /**
   Wait for the user to hit a key on the keyboard -- your application
   goes to sleep until a key is pressed and released on the keyboard.
   */
  // 
  ...
  //----------------------------------------------------------------------------  
  /**
   Wait for the user to press a key on the keyboard -- your application
   goes to sleep until a key is pressed.
   */
  // 
  ...
  /**
   ...
   If multiple keys are hit at the same time, the order of the keys
   returned is indeterminate. Key hits are bufferred. Therefore, if
   multiple keys are hit at the same time, subsequent calls to
   getKeyHitCode will return the other keys. A "key hit" will only be
   returned once.

   @return returns a key code for the latest key that was pressed and
   released by a user.

   <p>The integer values represents the key that was pressed (and not
   the upper or lower case of the key). <ul> <li>For the digits, the
   codes are equivalent to the Unicode character '0' - '9'</li> <li>For
   the characters, the codes are equivalent to the Unicode characters
   'A' - 'Z' (NOTE: CAPITAL LETTERS)</li> <li>For the function keys, use
   the named constants F1_KEY, ... F12_KEY</li> <li>For the other
   special keys, use: <ul> <li> DrawingPanel.LEFT_ARROW_KEY </li> <li>
   DrawingPanel.RIGHT_ARROW_KEY </li> <li> DrawingPanel.UP_ARROW_KEY
   </li> <li> DrawingPanel.DOWN_ARROW_KEY </li> <li>
   DrawingPanel.INSERT_KEY </li> <li> DrawingPanel.HOME_KEY </li> <li>
   DrawingPanel.DELETE_KEY </li> <li> DrawingPanel.PAGE_UP_KEY </li>
   <li> DrawingPanel.PAGE_DOWN_KEY </li> <li> DrawingPanel.ESC_KEY </li>
   <li> DrawingPanel.TAB_KEY </li> <li> DrawingPanel.SHIFT_KEY </li>
   <li> DrawingPanel.ENTER_KEY </li> </ul> </li> </ul>
  ...
  //----------------------------------------------------------------------------  
  /**
   Return the number of keys that are currently being pressed on the
   keyboard. <p>If you call the getKeyCode() the number of times
   returned by this method, you will get the key codes for all keys
  ...
  //----------------------------------------------------------------------------  
    /*
   Save the current graphics (in the offscreen buffer) to a file. The
   file name should have an appropriate image file extension, such as
   ".bmp" or ".jpg"
   */
  ...
  //----------------------------------------------------------------------------  
   ...
   @param filename the name of the bitmap image file (including the .bmp
   extension)
