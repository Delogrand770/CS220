...
public class ShannonSwitchingGraph {
  ...
  public boolean shortWins() {
    ...
    return path.getPath() == null ? false : path.getPath().getWeight() < 1.0;
  }
  ...
  public void makeShortMove() {
    ...
    //get the current shortest path between the two points
    DijkstraShortestPath path = new DijkstraShortestPath(graph, start, end);
    java.util.List edge = path.getPathEdgeList();
    ArrayList<DefaultWeightedEdge> edges = new ArrayList<>();

    //get an edge to highlight based on the shortest path
    ...
    //set the graph according to the edge that was just clicked
  ...
  public void makeCutMove() {
    ...
    //get an edge to highlight based on the shortest path and remove
    for (Iterator it = edge.iterator(); it.hasNext();) {
  ...
  public void removeLine(DefaultWeightedEdge e) {
    ...
    //get the points of the edge to remove
    Point v1 = graph.getEdgeSource(e);
    Point v2 = graph.getEdgeTarget(e);

    //find it in the arrayList fo lines and remove it from the list
  ...
  public void paint(Graphics graphics) {
    ...
    g2d.drawString(Client.turn == Client.SHORT ? "Short's Move" : "Cut's Move", 10, 15);
