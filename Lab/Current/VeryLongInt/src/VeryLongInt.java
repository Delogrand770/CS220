...
public class VeryLongInt {
  ...
  /**
   ...
   given String object. The worstTime(n) is O(n), where n represents the
   number of characters in s. There are no leading zeroes, except for 0
  ...
  /**
   Returns a String representation of this VeryLongInt object. The
   worstTime(n) is O(n), where n represents the number of digits in this
   VeryLongInt object.

   @return a String representation of this VeryLongInt object in the
   form [ followed by the digits, separated by commas and single spaces,
   followed by ].
  ...
  public boolean less(VeryLongInt OtherVeryLong) {
    if (this.digits.size() < OtherVeryLong.digits.size()) {
      return true;
    } else if (this.digits.size() > OtherVeryLong.digits.size() || this.equals(OtherVeryLong)) {
      return false;
    } else {
      for (int i = 0; i < this.digits.size(); i++) {
        if (this.digits.get(i) > OtherVeryLong.digits.get(i)) {
          return false;
        }
      }
    }
  ...
  public boolean greater(VeryLongInt OtherVeryLong) {
   return (!this.equals(OtherVeryLong) && !this.less(OtherVeryLong));
  }

  /**
   Increments this VeryLongInt object by a specified VeryLongInt object.
   The worstTime(n) is O(n), where n is the number of digits in the
   larger of this VeryLongInt object (before the call) and the specified
   VeryLongInt object.

   @param otherVeryLong - the specified VeryLongInt object to be added
  ...
  /**
   Returns the ith least significant digit in digits if i is a
   non-negative int less than digits.size(). Otherwise, returns 0.

   @param i - the number of positions from the right-most digit in
   digits to the digit sought.

   @return the ith least significant digit in digits, or 0 if there is
   no such digit.
  ...
  public boolean equals(VeryLongInt OtherVeryLongInt) {
    if (this.size() == OtherVeryLongInt.size()) {
      for (int i = 0; i < this.size(); i++) {
        if (this.digits.get(i) != OtherVeryLongInt.digits.get(i)) {
          return false;
        }
      }
      return true;
